# Each game must have a player and a goal.
# Use game.spawnPlayerXY("captain", x, y)
# to add a player to your game:
player = game.spawnPlayerXY("captain", 16, 30)
# Use game.addMoveGoalXY(x, y)
# to add a movement goal to your game:
# it should be 10m away from the player.
game.addMoveGoalXY(46, 30)
# If you want to, use spawnXY("fence", x, y)
# to make a maze with fences...
game.spawnXY("fence", 36, 12)
game.spawnXY("fence", 36, 16)
game.spawnXY("fence", 36, 20)
game.spawnXY("fence", 36, 24)
game.spawnXY("fence", 36, 28)
game.spawnXY("fence", 36, 32)
game.spawnXY("fence", 36, 36)
game.spawnXY("fence", 36, 40)
game.spawnXY("fence", 36, 44)
game.spawnXY("fence", 26, 6)
game.spawnXY("fence", 26, 10)
game.spawnXY("fence", 26, 14)
game.spawnXY("fence", 26, 18)
game.spawnXY("fence", 26, 22)
game.spawnXY("fence", 26, 26)
game.spawnXY("fence", 26, 30)
game.spawnXY("fence", 26, 34)
game.spawnXY("fence", 26, 38)
# Then, click "Test Level" to try your first playable game!
